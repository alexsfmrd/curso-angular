...
import { Actividad, EstadoActividad, Prioridad, esColeccionActividades } from '../models/actividad';
...
export class ActividadesService {
  ...
  crear(titulo: string, prioridad: Prioridad): void {
    const tituloLimpio = titulo.trim();
    if (tituloLimpio.length <3 || tituloLimpio.length > 80) {
      this.aviso.set('El título debe tener entre 3 y 80 caracteres.');
      return;
    }
